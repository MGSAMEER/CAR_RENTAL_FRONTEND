...
const CalendarHeader = () => (
  <header className="flex w-full items-center gap-1 pb-1">
    ...
    <HeadingRac className="grow text-center text-sm font-medium text-slate-900 dark:text-white" />
    <Button
      slot="next"
      className="flex size-9 items-center justify-center rounded-lg text-muted-foreground/80 outline-offset-2 transition-colors hover:bg-accent hover:text-foreground focus:outline-none data-[focus-visible]:outline data-[focus-visible]:outline-2 data-[focus-visible]:outline-ring/70"
    >
  ...
  return (
    <CalendarGridRac>
      <CalendarGridHeaderRac>
        {(day) => (
          <CalendarHeaderCellRac className="size-9 rounded-lg p-0 text-xs font-medium text-slate-500 dark:text-slate-400">
            {day}
      ...
      <CalendarGridBodyRac className="[&_td]:px-0">
        {(date) => (
          <CalendarCellRac
            date={date}
            className={cn(
              "relative flex size-9 items-center justify-center whitespace-nowrap rounded-lg border border-transparent p-0 text-sm font-normal text-foreground outline-offset-2 duration-150 [transition-property:color,background-color,border-radius,box-shadow] focus:outline-none data-[disabled]:pointer-events-none data-[unavailable]:pointer-events-none data-[focus-visible]:z-10 data-[hovered]:bg-accent data-[selected]:bg-primary data-[hovered]:text-foreground data-[selected]:text-primary-foreground data-[disabled]:opacity-30 data-[focus-visible]:outline data-[focus-visible]:outline-2 data-[focus-visible]:outline-ring/70",
              // Unavailable styles (Red Highlight)
              "data-[unavailable]:bg-red-50 data-[unavailable]:text-red-500 data-[unavailable]:border-red-100 data-[unavailable]:opacity-100 dark:data-[unavailable]:bg-red-950/30 dark:data-[unavailable]:text-red-400 dark:data-[unavailable]:border-red-900/30",
